...
class Graph
{
    private:
        city *cities;
        int **distmatrix; //Some implementations pass a pointer to pointer parameter to allow changing the head pointer directly instead of returning the new one
        int tcost;
...
void tspback_recursive(unsigned int *best_tour, unsigned int *best_tour_cost, unsigned int *partial_tour,unsigned int *partial_tour_cost, unsigned int level);
...
void Graph::displaycities()                             //display cities being called
{
    int i;
    for(i=0;i<numcities;i++)                               //num cities - public
    cout<<"\n"<<cities[i].name;                            //cities - private
}



void Graph::displaymatrix()                                //display matrix - public
{
    int i,j;
    for(i=0;i<numcities;i++)                               // num cities - public
    {
        ...
        {
        cout<<distmatrix[i][j]<<"\t";                      // distmatrix - private,pointer
        }
    cout<<endl;
    }
}
...
void Graph::CreateGraph()                                  // create graph - public
{
    int i,j,n;
    char tempch[15];
    int wrongcity=0;
    tcost=0;                                               // tcost - private
    cout<<"\n Enter Number of Cities :";
    cin>>n;
    numcities=n;
    cities=new city[n];                                    // cities - private
for(i=0;i<n;i++)
{
    cities[i].visited=0;
    cities[i].index=i;
...
do
{
    ...
    cout<<"Want to find the route(y/n)?:";
    cin>>choice;
    count++;
}
while(choice!='y');
...
void Graph::tspnn(int start)                                //tspnn - public
{
...
{
    cout<<"\n Optimal Path Using Greedy Nearest Neighbor Algorithm";
    cout<<"\n Optimal Path from Starting City "<<cities[start].name<<" Index:"<<start<<"\n";
    int totdist=0,i;
    cout<<"\n PATH:";
...
void Graph::tspback_recursive(unsigned int *best_tour, unsigned int *best_tour_cost, unsigned int *partial_tour,unsigned int *partial_tour_cost, unsigned int level)
    {
    ...
    *best_tour_cost = tour_cost;
    ...
    {
    if (*best_tour_cost == 0 || *partial_tour_cost + adjmat[partial_tour[level - 1]][partial_tour[i]]
    < *best_tour_cost)
    {
    /* Next permutation */
    ...
    *partial_tour_cost += cost;
    tspback_recursive(best_tour, best_tour_cost,partial_tour, partial_tour_cost, level + 1);
    *partial_tour_cost -= cost;
    swap(&partial_tour[level], &partial_tour[i]);
   }
   }
